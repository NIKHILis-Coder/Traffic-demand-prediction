...
**Honest caveat on the word "all" above:** the 3 base models' OOF predictions were genuinely OOF from the start. The *stacked* number was not, until this pass — see [§7 limitation #8](#7-limitations-honest-version) for the bug and the fix.
...
8. **The stacked "OOF" number wasn't actually out-of-fold — found and fixed this pass.** In `step10_model_training.ipynb` and `step10_part2_training.ipynb` (cell `fe3d2d8a`), the Ridge meta-learner was `.fit(oof_stack, y)` on the *full* set of base-model OOF predictions and then `.predict()`'d on that same `oof_stack` — that's a train-fit score, not an OOF score, even though it's one step removed from the base models (the base models' own predictions were still genuinely OOF). The notebook also computed a second, honest number alongside it via `cross_val_score`, and by coincidence both rounded to the same 0.0263 RMSE — but that's luck, not something the code guaranteed, and a stronger or less-regularized meta-learner could easily have shown a real gap between the two. **Fix applied:** replaced the fit-then-predict-on-same-data step with `cross_val_predict` (5-fold), so `oof_stacked_pred` — and everything downstream of it (the residual stats in [§6](#6-evaluation), the scatter plot, the reported RMSE/R²) — is now genuinely out-of-fold. The Ridge coefficients themselves are still fit on the *full* OOF stack afterward, which is correct: the final deployed meta-learner should use every available row, it's only the *evaluation number* that has to stay OOF. **I have not yet re-run the corrected cells** — doing so means retraining 15 gradient-boosted models (5 folds × 3 base learners), and per limitation #2 above I don't currently trust this environment's retraining numbers enough to swap them in casually. The code is correct; the artifacts on disk still reflect the old (numerically near-identical, but methodologically leaky) run.

9. **Final full-data models had no early-stopping signal at all — found and fixed this pass.** In the same two notebooks (cell `bb77a2f7`), the final XGBoost/LightGBM/CatBoost models were re-fit on 100% of `X`/`y` using the raw Optuna-tuned `n_estimators`/`iterations` (e.g. LightGBM's 2,643) with no `eval_set` and no early stopping. That parameter value is a *search-budget ceiling* the Optuna trial was allowed to spend, actually used only up to whatever early stopping (against a validation fold) cut it off at during CV — it was never validated as the right tree count for a model with no validation set. Concretely: LightGBM's Optuna trials were capped at 2,643 trees but each CV fold likely stopped well short of that against its own held-out fold; blindly training all 2,643 trees on the full data with nothing to stop it is a real overfitting risk on the *actual deployed model*, even though it wouldn't show up in the (fold-validated) OOF numbers at all — the OOF loop already uses early stopping correctly. **Fix applied:** the 5-fold OOF loop already trains and keeps one model per fold (`xgb_models`, `lgb_models`, `cat_models`), each of which knows its own early-stopped `best_iteration`. The final-fit cell now averages those five per-fold best iterations per model family and uses that as a fixed tree count for the full-data fit — no validation split needed for this step, and the final model's capacity now matches what CV actually showed generalizes, instead of an unvalidated upper bound. **This isn't just theoretical** — the *old* cell's committed output (before I cleared it) shows LightGBM logging `[LightGBM] [Warning] No further splits with positive gain, best gain: -inf` on repeat for most of its 2,643-tree budget, i.e. LightGBM itself was flagging that it had run out of useful splits well before the end — direct evidence the uncapped final fit was building far more trees than the data supported. **Also not yet re-run**, same reasoning as #8 — the currently-saved `.pkl` models in `models/` predate this fix.
...
- Re-run the full training pipeline in a version-pinned environment (`requirements.txt` now exists for exactly this reason) to get a trustworthy before/after comparison for the `geohash_prefix_enc` fix, and for the two `step10` fixes in #8/#9 above — all three land in the same expensive retraining step, so they should be verified together in one pinned-environment run rather than three separate ones.
